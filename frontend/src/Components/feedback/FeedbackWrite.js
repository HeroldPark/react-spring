...
function FeedbackWrite(props) {
    console.log("[FeedbackWrite.js] 시작 : " + props.postId);

    const { headers, setHeaders } = useContext(HttpHeadersContext);
    const postId = props.postId; // props로부터 postId 추출
    const title = props.title; // props로부터 title 추출
    ...
    const createFeedback = async() => {
        const req = {
            ...
            title: title, // props로부터 title 추출
            content: content,
        }

        await axiosInstance.post(`/feedback/write.do`, req, {headers: headers})
        .then((resp) => {
